...
def train_mle_estimator(train_images, train_labels):
    ...
    # Count of each class
    count_label = np.sum(train_labels, axis=0)

    # For calculating theta, since we want to count how many images in class
    # c has x_j = 1, we could use multiply wisely images and
    # labels to perform that, and use the existing num_label to calculate
    theta_mle = np.matmul(train_images.T, train_labels) / count_label

    # For calculating pi, simply just divide count of each class with
    # the total number of data
    pi_mle = count_label / train_images.shape[0]
...
def train_map_estimator(train_images, train_labels):
    ...
    # YOU NEED TO WRITE THIS PART
    # same to MLE, but only difference on theta_map that we add the
    # pseudo-count to the fraction
    count_label = np.sum(train_labels, axis=0)
    theta_map = (2 + np.matmul(train_images.T, train_labels)) / (4 + count_label)
    pi_map = count_label / train_images.shape[0]
...
def log_likelihood(images, theta, pi):
    ...
    # YOU NEED TO WRITE THIS PART
    # the numerator part of p(c |x^(i), theta, pi) under log
    temp1 = np.log(pi) + np.dot(images, np.log(theta)) + np.dot(1.0 - images, np.log(1.0 - theta))
    # the denominator part of p(c|x^(i), theta, pi) under log
    sum_temp = np.sum(np.exp(temp1), axis=1)
    temp2 = np.log(sum_temp)
    # the log likelihood
    log_like = (temp1.T - temp2).T
    return log_like
...
def accuracy(log_like, labels):
    ...
    acc = np.sum(pred == np.argmax(labels, axis=1)) / pred.shape[0]
    return acc
